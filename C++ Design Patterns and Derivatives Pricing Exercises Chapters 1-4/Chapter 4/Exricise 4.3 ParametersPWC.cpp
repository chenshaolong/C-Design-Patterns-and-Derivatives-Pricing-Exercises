...
ParametersPWC::ParametersPWC(const MJArray& times_, const MJArray& constant_)
:Times(times_),Constant(constant_)
{
...
double ParametersPWC::Integral(double time1, double time2) const
{
    ...
    for (unsigned long i = _time1; i < _time2; ++i)
        integral += K1[i];
