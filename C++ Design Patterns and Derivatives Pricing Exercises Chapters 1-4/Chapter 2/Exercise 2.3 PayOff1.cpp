...
PayOff::PayOff(const double Strike_, const double Strike2_, OptionType TheOptionsType_) : Strike(Strike_), Strike2(Strike2_), TheOptionsType(TheOptionsType_) {}

double PayOff::operator()(const double spot) const {
    switch (TheOptionsType) {
    case call :
        return std::max(spot-Sconst trike, 0.0);
    case put:
